<div class="container">
	<h3 class="section-heading text-tengah header-phylosoper home-header">Upgrade Layanan</h3>
	...
	<div class="row">
		<div class="text-tengah gray-wrap-content">
			...
			<div class="p_t_b_10" id="form-upgrade">
				...
				<form action="<?php echo $profileInqEndpoint;?>" method="post" role='form' class="form-center">
					<input type="hidden" id='msisdn' name="msisdn" value="123123123"/>
					<div class="input-group pin-wrap">
                        <span class="input-group-addon icon-login">
                            <i class="glyphicon glyphicon-lock"></i>
                        </span>
                        <input id="pin" type="password" class="form-control" name="pin" minlength="6" maxlength="6" value="" placeholder="PIN" required/>
                    </div>
                    <button type="submit" name="sbm_upgrade" class="btn btn-header text-menu m_t_10 bg-white">
                    	<span>Lanjut</span>
                    </button>
				</form>
			</div>
			...
			<div class="p_t_b_10" id="init_upgrade">
				 <h4 class="text-gray">Silahkan melakukan pencarian no ecash yang ingin diupgrade.</h4>
			</div>
			<div id="loading">
							...
							$.ajax({
            					...
            					success:function(data){
            						...
            						if(resp.status == "<?php echo STATUS_PROCESSED;?>"){
            							if(resp.groupId == "<?php echo $registered ?>" || resp.groupId == "<?php echo $pendingRegister;?>" || resp.groupId == "<?php echo $pendingRegisterAgent;?>"){
            								state=0;
											$('#loading').fadeOut('500',function(){
                        						$('#modal-content').html("<div class='container'><button role='btn' class='close-alert' id='close-btn'  onclick='exitToggle()'><i class='fa fa-close text-white'></i></button><div class='header-alert'><h2><?php echo HEAD_CANNOT_UPGRADE;?></h2></div><h4><?php echo DESC_CANNOT_UPGRADE;?></h4></div>");
