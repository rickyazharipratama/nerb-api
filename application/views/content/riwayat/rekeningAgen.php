<div class="container">
    <h3 class="section-heading text-tengah header-phylosoper home-header">Riwayat Rekening Agen</h3>
    ...
    <div class="row clearfix">
        ...
        <div id="history-error">
            <div class="text-tengah m-t-5-b-20">
                ...
                <h4 class="text-gray" id="DescErrorHistory">Belum ada riwayat transaksi.</h4>
            </div>
        </div>
                  ...
                  if(resp.status == "<?php echo STATUS_TOKEN_EXPIRED;?>"){
                    ...
                    if(resp.status=="<?php echo STATUS_PROCESSED;?>"){
                        ...
                        for(var i = 0; i < history.length;i++){
                            ...
                            var icon = "";
                            if(history[i].transferTypeID == "<?php echo TRANS_TYPE_ID_SETOR_TUNAI?>"){
                                icon = "<img src='<?php echo base_url();?>images/setormini.png' alt=''/>";
                            }else if(history[i].transferTypeID == "<?php echo TRANS_TYPE_ID_REAL_TRX_TUNAI;?>" ||
                                    history[i].transferTypeID == "<?php echo TRANS_TYPE_ID_BIAYA_AGEN;?>" || 
                                    history[i].transferTypeID == "<?php echo TRANS_TYPE_ID_BIAYA_NASABAH_AGEN;?>"){
                                icon = "<img src='<?php echo base_url();?>images/tarikmini.png' alt=''/>";
                            }else{
                                icon = "<img src='<?php echo base_url();?>images/transaksimini.png' alt=''/>";
                            } 
                            var amt = "";
                            if(history[i].amount.indexOf("-")!== -1){
                                amt = "<span class='timeline-amount color-red'>"+history[i].amount+"</span>";
                            }else{
                                amt = "<span class='timeline-amount'>"+history[i].amount+"</span>";
                            }                           
                            if(isCantMarg){
                                if(i % 2 == 0){
                                    $("#setorTunaiList").append(
                                        "<li class='page-scroll'><div class='timeline-badge'>"+icon+"</div><div class='timeline-panel'><div class='timeline-heading'><h4 class='timeline-title'>"+history[i].description+"</h4><p class='no-marg'><small class='text-muted rise-size'><i class='fa fa-file-o'></i> "+ history[i].transactionNumber+"</small></p><p class='no-marg'><small class='text-muted rise-size'><i class='fa fa-clock-o'></i> "+ StringDate(history[i].transactionDate)+"</small></p><p class='no-marg'><small class='text-muted rise-size'><i class='fa fa-user-circle-o'></i> "+ history[i].username+" - "+history[i].name+"</small></p></div><div class='timeline-body'>"+amt+"</div></div></li>"
                                    );
                                    isMod = true;
                                }else{
                                    $("#setorTunaiList").append(
